    ...
    internal class ModInstance : MonoBehaviour
    {
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            if (scene.name.Equals("Mount Holly Estate"))
            {
                ...
                Logging.Log(String.Join("\n", Plugin.ModRoomManager.Rooms));
                Harmony.CreateAndPatchAll(typeof(RoomPatches), "RoomPatches");
                Plugin.ModRoomManager.ForceDraft("CLOSET");
            }
            else {
        ...
        private static void RemoveDay1Force(PlayMakerFSM fsm) {
            FsmState DraftCodeStart = fsm.GetState("Draft Code Start");
            DraftCodeStart.ChangeTransition("FINISHED", "SLOT 2");
            FsmState PickAnother = fsm.GetState("Pick Another ");
            PickAnother.ChangeTransition("FINISHED", "SLOT 2");
        }
        ...
        private void AddRoomForcer(PlayMakerFSM fsm)
        {
            ...
            FsmTransition ContinueDraft = ForceDraft.AddTransition("Continue Draft", "SLOT 2");
            ForceDraft.AddLastAction(new BoolTest() { boolVariable = isDraftForced, isFalse = FsmEvent.GetFsmEvent("Continue Draft"), isTrue = null, everyFrame = false });
            ForceDraft.AddLastAction(new SetFsmGameObject() { variableName = "RoomEngine", gameObject = null,  everyFrame = false});
            ForceDraft.AddLastAction(new SendEvent() { sendEvent = FsmEvent.GetFsmEvent("PLAN SELECTED"), delay = 0f, everyFrame = false });
            ForceDraft.AddTransition("Continue Draft", "SLOT 2");
        }
