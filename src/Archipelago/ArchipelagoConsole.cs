...
public static class ArchipelagoConsole
{
    ...
    {
        UpdateWindow();
        Logging.LogWarning("Waking Up Console");
    }
...
#endif
#if ML
    if (Hidden && Input.GetKeyInt(UnityEngine.KeyCode.Slash))
        {
            Hidden = !Hidden;
            UpdateWindow();
        }
        if (!Hidden && Input.GetKeyInt(UnityEngine.KeyCode.Escape))
        {
            Hidden = !Hidden;
            UpdateWindow();
        }
        ...
        if (!string.IsNullOrWhiteSpace(CommandText) && (GUI.Button(SendCommandButton, "Send") || e.type == EventType.KeyDown && (e.keyCode == UnityEngine.KeyCode.Return || e.character == '\n')))
        {
            //local command
